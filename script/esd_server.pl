...
BEGIN {
    ...
    $ENV{LD_LIBRARY_PATH} = "/tdr/bin/lib:/usr/local/lib";
...
use lib "/tdr/bin/esd/lib";

my $debug             = 0;
my $fork              = 0;
my $help              = 0;
my $host              = undef;
my $port              = $ENV{ESD_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{ESD_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
drop_server.pl - Catalyst Testserver

=head1 SYNOPSIS

drop_server.pl [options]
